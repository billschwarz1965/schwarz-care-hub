import"./modulepreload-polyfill-B5Qt9EMX.js";const N="/api/pubmed",te='"N Engl J Med"[Journal]',w=document.getElementById("search-input"),P=document.getElementById("search-btn"),J=document.getElementById("results-area"),q=document.getElementById("intel-panel"),A=document.getElementById("signal-feed"),ne=document.getElementById("governance-bar"),R=document.getElementById("conn-pubmed"),Y=document.getElementById("conn-nejm"),D=document.getElementById("compare-panel-area"),S=document.getElementById("compare-bar"),U=document.getElementById("memory-section"),G=document.getElementById("memory-chips"),Q=document.getElementById("memory-clear");let _="all",z=[],X=0,$=[];const H="medverse_lit_memory";document.querySelectorAll(".source-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".source-btn").forEach(a=>a.classList.remove("active")),e.classList.add("active"),_=e.dataset.source,z.length&&ae(z)})});document.querySelectorAll(".quick-tag").forEach(e=>{e.addEventListener("click",()=>{w.value=e.dataset.q,C()})});P.addEventListener("click",C);w.addEventListener("keydown",e=>{e.key==="Enter"&&C()});le();async function le(){try{(await fetch(`${N}/einfo.fcgi?db=pubmed&retmode=json`)).ok&&(R.innerHTML='<span class="conn-dot pulse"></span> PubMed E-Utilities — Connected',R.className="conn-chip")}catch{R.innerHTML='<span class="conn-dot"></span> PubMed — Connection failed',R.className="conn-chip error"}Y.innerHTML='<span class="conn-dot pulse"></span> NEJM via PubMed — Connected',Y.className="conn-chip"}async function C(){const e=w.value.trim();if(e){P.disabled=!0,P.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Searching…',ne.style.display="flex",J.innerHTML=`<div class="search-loading">
  ...
  </div>`);try{const a=await O(e,15);await g(500);const t=await O(`${e} AND ${te}`,5),i=new Set(t.map(s=>s.uid)),n=[];t.forEach(s=>{s.isNejm=!0,n.push(s)}),a.forEach(s=>{i.has(s.uid)||(s.isNejm=!1,n.push(s))}),z=n,$=[],F(),ae(n),ce(e,n,a.length,t.length),ie(e,n),ue(e,n.length,t.length),ve(n)}catch(a){J.innerHTML=`<div class="empty-state">
      <i class="ti ti-alert-triangle"></i>
      <h3>Search error</h3>
      <p>${o(a.message)}</p>
    </div>`}P.disabled=!1,P.innerHTML='<i class="ti ti-search"></i> Search'}}async function O(e,a=15){var c,p;const t=`${N}/esearch.fcgi?db=pubmed&term=${encodeURIComponent(e)}&retmax=${a}&sort=relevance&retmode=json`,s=((c=(await(await fetch(t)).json()).esearchresult)==null?void 0:c.idlist)||[];if(!s.length)return[];const l=`${N}/esummary.fcgi?db=pubmed&id=${s.join(",")}&retmode=json`,h=await(await fetch(l)).json(),u=[];for(const b of s){const d=(p=h.result)==null?void 0:p[b];d&&u.push({uid:d.uid,title:d.title||"Untitled",authors:(d.authors||[]).map(m=>m.name),source:d.source||"",fulljournalname:d.fulljournalname||d.source||"",pubdate:d.pubdate||"",volume:d.volume||"",issue:d.issue||"",pages:d.pages||"",doi:oe(d.elocationid||""),pmid:d.uid})}return u}function oe(e){const a=e.match(/doi:\s*(\S+)/i);return a?a[1]:""}function ae(e){let a=e;if(_==="nejm"?a=e.filter(s=>s.isNejm):_==="pubmed"&&(a=e.filter(s=>!s.isNejm)),!a.length){J.innerHTML=`<div class="empty-state">
  ...
  `;const n=document.getElementById("results-list");a.forEach((s,l)=>{const r=document.createElement("div");r.className="article-card";const h=s.authors.length>4?s.authors.slice(0,3).join(", ")+`, et al. (${s.authors.length} authors)`:s.authors.join(", "),u=s.isNejm?"nejm":"pubmed",c=s.isNejm?"NEJM":"PubMed",p=`https://pubmed.ncbi.nlm.nih.gov/${s.pmid}/`,b=s.doi?`https://doi.org/${s.doi}`:"";r.innerHTML=`
    ...
    `,n.appendChild(r)}),n.querySelectorAll(".fetch-abstract-btn").forEach(s=>{s.addEventListener("click",()=>re(s.dataset.pmid))}),n.querySelectorAll(".compare-check").forEach(s=>{s.addEventListener("click",()=>pe(s.dataset.pmid,s))})}async function re(e){const a=document.getElementById(`abstract-${e}`);if(a){a.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:8px 0"><i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Fetching abstract from PubMed…</p>';try{const t=`${N}/efetch.fcgi?db=pubmed&id=${e}&rettype=abstract&retmode=text`,n=await(await fetch(t)).text(),s=n.match(/\n\n([\s\S]*?)(?:\n\n(?:Copyright|DOI|PMID|©)|\n\nPMID:)/i),l=s?s[1].trim():n.trim();l&&l.length>50?a.innerHTML=`<div class="article-abstract" style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border)">${o(l)}</div>
        <button class="article-expand" onclick="this.previousElementSibling.classList.toggle('expanded');this.textContent=this.textContent==='Show more'?'Show less':'Show more'">Show more</button>`:a.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:8px 0">Abstract not available for this article.</p>'}catch{a.innerHTML='<p style="font-size:12px;color:var(--danger);padding:8px 0">Failed to fetch abstract.</p>'}}}function ce(e,a,t,i){const n={},s={},l={};a.forEach(m=>{const v=m.source||m.fulljournalname||"Unknown";n[v]=(n[v]||0)+1;const E=m.pubdate.match(/(\d{4})/);if(E){const f=E[1];s[f]=(s[f]||0)+1}m.authors.forEach(f=>{l[f]=(l[f]||0)+1})});const r=Object.entries(n).sort((m,v)=>v[1]-m[1]).slice(0,5),h=r.length?r[0][1]:1,u=Object.entries(l).sort((m,v)=>v[1]-m[1]).slice(0,8),c=Object.entries(s).sort((m,v)=>m[0]-v[0]),p=c.length?Math.max(...c.map(m=>m[1])):1,b=["#7a00e6","#aa46a3","#f9c851","#60a5fa","#34d399"];let d="";d+=`<div class="intel-card">
    ...
    <div class="intel-stat-row"><span class="intel-stat-label">Date range</span><span class="intel-stat-value">${c.length?c[0][0]+"–"+c[c.length-1][0]:"—"}</span></div>
      ...
      ${r.map(([m,v],E)=>`
        <div class="journal-row">
          ...
          <div class="journal-bar"><div class="journal-bar-fill" style="width:${v/h*100}%;background:${b[E%b.length]}"></div></div>
  ...
  </div>`,q&&(q.innerHTML=d)}function ie(e,a){X++;const t=a.filter(c=>c.isNejm).length,n=new Date().toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",second:"2-digit"}),s=(()=>{const c={};a.forEach(b=>b.authors.forEach(d=>c[d]=(c[d]||0)+1));const p=Object.entries(c).sort((b,d)=>d[1]-b[1]);return p.length?p[0][0]:"Unknown"})(),l=de(e),r=t>0?"Deep engagement — cross-journal query":"Moderate engagement",h=t>0?"Evidence synthesis with high-impact journals":"Literature exploration",u=t>2?"PRIORITY: High-impact NEJM evidence discovered — flag for medical affairs review":`Literature search logged — ${a.length} articles, key author: ${s}`;if(A){A.querySelector("[style]")&&X===1&&(A.innerHTML="");const p=document.createElement("div");p.className="signal-card",p.innerHTML=`
    ...
    `,A.insertBefore(p,A.firstChild)}}function de(e){const a=e.toLowerCase(),t=[[/atopic|eczema|dermatitis|dupilumab|dupixent/,"Atopic Dermatitis"],[/asthma|airway|tezepelumab/,"Asthma"],[/crohn|ibd|ulcerative colitis|duvakitug/,"Inflammatory Bowel Disease"],[/rsv|nirsevimab|beyfortus/,"RSV Prevention"],[/hemophilia|fitusiran/,"Hemophilia"],[/multiple sclerosis|tolebrutinib/,"Multiple Sclerosis"],[/insulin|glargine|lantus|toujeo|diabetes/,"Diabetes"],[/psoriasis/,"Psoriasis"],[/oncology|cancer|tumor/,"Oncology"],[/vaccine|immunization/,"Vaccines"],[/rare disease|fabry|gaucher|pompe/,"Rare Diseases"]];for(const[i,n]of t)if(i.test(a))return n;return"Cross-TA / General"}function o(e){const a=document.createElement("div");return a.textContent=e,a.innerHTML}function g(e){return new Promise(a=>setTimeout(a,e))}document.getElementById("chat-panel");const j=document.getElementById("chat-messages"),y=document.getElementById("chat-input"),M=document.getElementById("chat-send"),L=document.getElementById("chat-suggestions");y&&M&&(M.addEventListener("click",()=>I()),y.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),I())}),y.addEventListener("input",()=>{M.disabled=!y.value.trim()}));L&&L.querySelectorAll(".chat-suggestion").forEach(e=>{e.addEventListener("click",()=>{y.value=e.dataset.q,I()})});const Z={patterns:[{match:/what.*(latest|recent|new).*(dupilumab|dupixent)/i,query:"dupilumab 2024 2025 2026",synthesize:e=>`Based on my search of PubMed and NEJM, here are the most recent dupilumab publications:
...
`),i=a.filter(l=>l.isNejm).length;let n="";const s=localStorage.getItem(H);if(s){const r=JSON.parse(s).filter(h=>h.query.toLowerCase()!==e.toLowerCase()).slice(0,2);r.length&&(n=`
...
${a.length>3?`Plus ${a.length-3} more articles. `:""}Use the search bar above to see all results with full metadata, or ask me a more specific follow-up question.${n}`}};async function I(){const e=y.value.trim();if(!e)return;y.value="",M.disabled=!0,L&&(L.style.display="none"),V("user",e);const a=be();let t=null;for(const n of Z.patterns)if(n.match.test(e)){t=n;break}const i=t?t.query:me(e);try{const n=await O(i,8);await g(300);const s=await O(`${i} AND ${te}`,3),l=new Set(s.map(u=>u.uid));s.forEach(u=>{u.isNejm=!0}),n.forEach(u=>{l.has(u.uid)&&(u.isNejm=!0)});const r=[...s,...n.filter(u=>!l.has(u.uid))];a.remove();const h=t?t.synthesize(r):Z.fallback(e,r);V("agent",h,r),ie(i,r)}catch(n){a.remove(),V("agent",`I encountered an error searching PubMed: ${n.message}. Please try again in a moment — NCBI rate-limits requests to 3 per second.`)}}function me(e){return e.replace(/\b(what|how|does|is|are|the|can|you|find|tell|me|about|give|show|latest|recent|any|published|evidence|data|for|on|in|of|with|and|or|please)\b/gi,"").replace(/[?!.,]/g,"").replace(/\s+/g," ").trim()}function V(e,a,t){const i=document.createElement("div");if(i.className=`chat-msg chat-msg-${e}`,e==="user")i.innerHTML=`<div class="chat-msg-bubble chat-user-bubble">${o(a)}</div>`;else{let n=o(a);n=n.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),n=n.replace(/\*(.+?)\*/g,"<em>$1</em>"),n=n.replace(/\[PMID: (\d+)\]/g,'<a class="chat-pmid-link" href="https://pubmed.ncbi.nlm.nih.gov/$1/" target="_blank" rel="noopener">PMID: $1</a>'),n=n.replace(/^(\d+)\./gm,"<br>$1."),n=n.replace(/^- (.+)/gm,"<br>• $1");const s=t&&t.length>0?`<div class="chat-citations">
      ...
      <div class="chat-msg-content">
        ...
        ${s}
      </div>`}j.appendChild(i),j.scrollTop=j.scrollHeight}function K(){const e=localStorage.getItem(H),a=e?JSON.parse(e):[];if(!a.length){U&&(U.style.display="none");return}U.style.display="block",G.innerHTML="",a.slice(0,8).forEach(t=>{const i=document.createElement("span");i.className="memory-chip",i.title=`${t.count} results · ${t.nejm} NEJM · ${t.date}`,i.innerHTML=`<i class="ti ti-history"></i> ${o(t.query)} <span class="memory-chip-time">${t.ago}</span>`,i.addEventListener("click",()=>{w.value=t.query,C()}),G.appendChild(i)})}function ue(e,a,t){const i=localStorage.getItem(H);let n=i?JSON.parse(i):[];n=n.filter(l=>l.query.toLowerCase()!==e.toLowerCase());const s=new Date;n.unshift({query:e,count:a,nejm:t,date:s.toLocaleDateString("en-US",{month:"short",day:"numeric"}),ago:"just now",ts:s.getTime()}),n=n.slice(0,20),n.forEach(l=>{const r=Math.floor((s.getTime()-l.ts)/6e4);r<1?l.ago="just now":r<60?l.ago=`${r}m ago`:r<1440?l.ago=`${Math.floor(r/60)}h ago`:l.ago=`${Math.floor(r/1440)}d ago`}),localStorage.setItem(H,JSON.stringify(n)),K()}K();Q&&Q.addEventListener("click",()=>{localStorage.removeItem(H),K()});function pe(e,a){var i;const t=$.indexOf(e);if(t>-1)$.splice(t,1),a.classList.remove("selected");else{if($.length>=2){const n=$.shift();(i=document.querySelector(`.compare-check[data-pmid="${n}"]`))==null||i.classList.remove("selected")}$.push(e),a.classList.add("selected")}F()}function F(){if(S){if($.length<2){S.style.display=$.length===1?"flex":"none",$.length===1&&(S.innerHTML=`<div class="compare-bar">
  ...
  </div>`,S.style.display="block"}}window.clearCompare=function(){$=[],document.querySelectorAll(".compare-check.selected").forEach(e=>e.classList.remove("selected")),F()};window.runComparison=async function(){var v,E;const e=$.map(f=>z.find(x=>x.uid===f||x.pmid===f)).filter(Boolean);if(e.length<2)return;D.innerHTML=`<div class="compare-panel">
  ...
  </div>`,D.scrollIntoView({behavior:"smooth",block:"start"});const a=[];for(const f of e)try{const x=`${N}/efetch.fcgi?db=pubmed&id=${f.pmid}&rettype=abstract&retmode=text`,W=(await(await fetch(x)).text()).match(/\n\n([\s\S]*?)(?:\n\n(?:Copyright|DOI|PMID|©)|\n\nPMID:)/i);a.push(W?W[1].trim():"Abstract not available."),a.length<e.length&&await g(400)}catch{a.push("Failed to fetch abstract.")}const t=e[0],i=e[1],n=a[0],s=a[1],l=t.fulljournalname===i.fulljournalname,r=((v=t.pubdate.match(/\d{4}/))==null?void 0:v[0])||"?",h=((E=i.pubdate.match(/\d{4}/))==null?void 0:E[0])||"?",u=Math.abs(parseInt(r)-parseInt(h));let c="";if(l)c=`Both articles published in <strong>${o(t.fulljournalname)}</strong>`,u>0&&(c+=` — ${u}-year gap suggests evolving evidence base`),c+=". ";else{const f=t.isNejm,x=i.isNejm;f&&!x?c=`Article 1 is from <strong>NEJM</strong> (highest impact) vs ${o(i.fulljournalname)}. `:!f&&x?c=`Article 2 is from <strong>NEJM</strong> (highest impact) vs ${o(t.fulljournalname)}. `:c=`Cross-journal comparison: <strong>${o(t.source)}</strong> vs <strong>${o(i.source)}</strong>. `}const p=t.authors.length,b=i.authors.length;(p>10||b>10)&&(c+=`Large author list (${Math.max(p,b)} authors) suggests multi-center trial data. `),c+="Review both abstracts to assess study design, population, and endpoints for comparative evidence strength.";const d=f=>f.isNejm?"nejm":"pubmed",m=f=>f.isNejm?"NEJM":"PubMed";D.innerHTML=`<div class="compare-panel">
    <div class="compare-panel-header">
    ...
    <div class="compare-ai-row">
      ...
      <div class="compare-ai-text">${c}</div>
          ...
          </div>`}else n.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:8px 0"><span class="deep-dive-badge"><i class="ti ti-sparkles"></i> Deep-Dive</span> Abstract not available from PubMed for this article.</p>'}catch{n.innerHTML=""}}}}const T=document.getElementById("demo-search-btn");T&&T.addEventListener("click",ge);async function B(e,a){e.value="";for(let t=0;t<a.length;t++)e.value+=a[t],await g(20+Math.random()*25)}async function ge(){T.disabled=!0,T.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Running demo…',w.scrollIntoView({behavior:"smooth",block:"center"}),await g(600),await B(w,"dupilumab atopic dermatitis long-term safety"),await g(500),await C(),await g(4e3),w.scrollIntoView({behavior:"smooth",block:"center"}),await g(1e3),w.value="",await B(w,"nirsevimab RSV prevention infants"),await g(800),await C(),await g(2e3),T.disabled=!1,T.innerHTML='<i class="ti ti-player-play"></i> Watch literature search demo'}function se(){j.innerHTML=`<div class="chat-msg chat-msg-agent">
    <div class="chat-agent-avatar"><i class="ti ti-book-2"></i></div>
    <div class="chat-msg-content">
  ...
  </div>`,L.innerHTML=`<button class="chat-suggestion" data-q="What are the latest publications on dupilumab?">Latest dupilumab pubs</button>
    ...
    <button class="chat-suggestion" data-q="Tell me about tolebrutinib in multiple sclerosis">Tolebrutinib in MS</button>`,L.style.display="flex",y.value="",M.disabled=!0,L.querySelectorAll(".chat-suggestion").forEach(e=>{e.addEventListener("click",()=>{y.value=e.dataset.q,I()})})}const ee=document.getElementById("chat-clear-btn");ee&&ee.addEventListener("click",se);const k=document.getElementById("demo-chat-btn");k&&k.addEventListener("click",fe);async function fe(){k.disabled=!0,k.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Running demo…',se(),await g(600),await B(y,"What are the latest publications on dupilumab?"),M.disabled=!1,await g(500),await I(),await g(4e3),await B(y,"What evidence exists for nirsevimab RSV prevention?"),M.disabled=!1,await g(500),await I(),await g(4e3),await B(y,"Tell me about tolebrutinib in multiple sclerosis"),M.disabled=!1,await g(500),await I(),await g(2e3),k.disabled=!1,k.innerHTML='<i class="ti ti-player-play"></i> Watch conversation demo'}function be(){const e=document.createElement("div");return e.className="chat-msg chat-msg-agent",e.innerHTML=`
    <div class="chat-agent-avatar"><i class="ti ti-book-2"></i></div>
    <div class="chat-msg-content">
      <div class="chat-msg-bubble chat-agent-bubble">
