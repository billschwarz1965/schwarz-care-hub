import{i as le,s as oe,a as re,b as ce,h as de}from"./narrator-C-XhPlAK.js";const C="/api/pubmed",ae='"N Engl J Med"[Journal]',w=document.getElementById("search-input"),P=document.getElementById("search-btn"),q=document.getElementById("results-area"),z=document.getElementById("intel-panel"),A=document.getElementById("signal-feed"),me=document.getElementById("governance-bar"),J=document.getElementById("conn-pubmed"),G=document.getElementById("conn-nejm"),D=document.getElementById("compare-panel-area"),S=document.getElementById("compare-bar"),V=document.getElementById("memory-section"),Q=document.getElementById("memory-chips"),X=document.getElementById("memory-clear");let K="all",O=[],Z=0,$=[];const H="medverse_lit_memory";document.querySelectorAll(".source-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".source-btn").forEach(a=>a.classList.remove("active")),e.classList.add("active"),K=e.dataset.source,O.length&&ie(O)})});document.querySelectorAll(".quick-tag").forEach(e=>{e.addEventListener("click",()=>{w.value=e.dataset.q,N()})});P.addEventListener("click",N);w.addEventListener("keydown",e=>{e.key==="Enter"&&N()});ue();async function ue(){try{(await fetch(`${C}/einfo.fcgi?db=pubmed&retmode=json`)).ok&&(J.innerHTML='<span class="conn-dot pulse"></span> PubMed E-Utilities — Connected',J.className="conn-chip")}catch{J.innerHTML='<span class="conn-dot"></span> PubMed — Connection failed',J.className="conn-chip error"}G.innerHTML='<span class="conn-dot pulse"></span> NEJM via PubMed — Connected',G.className="conn-chip"}async function N(){const e=w.value.trim();if(e){P.disabled=!0,P.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Searching…',me.style.display="flex",q.innerHTML=`<div class="search-loading">
...
${a.length>3?`Plus ${a.length-3} more articles. `:""}Use the search bar above to see all results with full metadata, or ask me a more specific follow-up question.${n}`}};async function I(){const e=y.value.trim();if(!e)return;y.value="",M.disabled=!0,x&&(x.style.display="none"),_("user",e);const a=je();let t=null;for(const n of ee.patterns)if(n.match.test(e)){t=n;break}const i=t?t.query:fe(e);try{const n=await U(i,8);await g(300);const s=await U(`${i} AND ${ae}`,3),l=new Set(s.map(u=>u.uid));s.forEach(u=>{u.isNejm=!0}),n.forEach(u=>{l.has(u.uid)&&(u.isNejm=!0)});const r=[...s,...n.filter(u=>!l.has(u.uid))];a.remove();const h=t?t.synthesize(r):ee.fallback(e,r);_("agent",h,r),se(i,r)}catch(n){a.remove(),_("agent",`I encountered an error searching PubMed: ${n.message}. Please try again in a moment — NCBI rate-limits requests to 3 per second.`)}}function fe(e){return e.replace(/\b(what|how|does|is|are|the|can|you|find|tell|me|about|give|show|latest|recent|any|published|evidence|data|for|on|in|of|with|and|or|please)\b/gi,"").replace(/[?!.,]/g,"").replace(/\s+/g," ").trim()}function _(e,a,t){const i=document.createElement("div");if(i.className=`chat-msg chat-msg-${e}`,e==="user")i.innerHTML=`<div class="chat-msg-bubble chat-user-bubble">${o(a)}</div>`;else{let n=o(a);n=n.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),n=n.replace(/\*(.+?)\*/g,"<em>$1</em>"),n=n.replace(/\[PMID: (\d+)\]/g,'<a class="chat-pmid-link" href="https://pubmed.ncbi.nlm.nih.gov/$1/" target="_blank" rel="noopener">PMID: $1</a>'),n=n.replace(/^(\d+)\./gm,"<br>$1."),n=n.replace(/^- (.+)/gm,"<br>• $1");const s=t&&t.length>0?`<div class="chat-citations">
      ...
      <div class="chat-msg-content">
        ...
        ${s}
      </div>`}j.appendChild(i),j.scrollTop=j.scrollHeight}function F(){const e=localStorage.getItem(H),a=e?JSON.parse(e):[];if(!a.length){V&&(V.style.display="none");return}V.style.display="block",Q.innerHTML="",a.slice(0,8).forEach(t=>{const i=document.createElement("span");i.className="memory-chip",i.title=`${t.count} results · ${t.nejm} NEJM · ${t.date}`,i.innerHTML=`<i class="ti ti-history"></i> ${o(t.query)} <span class="memory-chip-time">${t.ago}</span>`,i.addEventListener("click",()=>{w.value=t.query,N()}),Q.appendChild(i)})}function be(e,a,t){const i=localStorage.getItem(H);let n=i?JSON.parse(i):[];n=n.filter(l=>l.query.toLowerCase()!==e.toLowerCase());const s=new Date;n.unshift({query:e,count:a,nejm:t,date:s.toLocaleDateString("en-US",{month:"short",day:"numeric"}),ago:"just now",ts:s.getTime()}),n=n.slice(0,20),n.forEach(l=>{const r=Math.floor((s.getTime()-l.ts)/6e4);r<1?l.ago="just now":r<60?l.ago=`${r}m ago`:r<1440?l.ago=`${Math.floor(r/60)}h ago`:l.ago=`${Math.floor(r/1440)}d ago`}),localStorage.setItem(H,JSON.stringify(n)),F()}F();X&&X.addEventListener("click",()=>{localStorage.removeItem(H),F()});function ye(e,a){var i;const t=$.indexOf(e);if(t>-1)$.splice(t,1),a.classList.remove("selected");else{if($.length>=2){const n=$.shift();(i=document.querySelector(`.compare-check[data-pmid="${n}"]`))==null||i.classList.remove("selected")}$.push(e),a.classList.add("selected")}W()}function W(){if(S){if($.length<2){S.style.display=$.length===1?"flex":"none",$.length===1&&(S.innerHTML=`<div class="compare-bar">
  ...
  </div>`,S.style.display="block"}}window.clearCompare=function(){$=[],document.querySelectorAll(".compare-check.selected").forEach(e=>e.classList.remove("selected")),W()};window.runComparison=async function(){var v,E;const e=$.map(f=>O.find(L=>L.uid===f||L.pmid===f)).filter(Boolean);if(e.length<2)return;D.innerHTML=`<div class="compare-panel">
    ...
    <button class="chat-suggestion" data-q="Tell me about tolebrutinib in multiple sclerosis">Tolebrutinib in MS</button>`,x.style.display="flex",y.value="",M.disabled=!0,x.querySelectorAll(".chat-suggestion").forEach(e=>{e.addEventListener("click",()=>{y.value=e.dataset.q,I()})})}const te=document.getElementById("chat-clear-btn");te&&te.addEventListener("click",ne);const k=document.getElementById("demo-chat-btn");k&&k.addEventListener("click",Le);function R(e){const a=document.getElementById("demo-narrator");a&&(a.innerHTML=`<i class="ti ti-sparkles"></i> ${e}`,le()&&a.classList.add("visible"),oe(e),re())}function Ee(){const e=document.getElementById("demo-narrator");e&&e.classList.remove("visible"),ce(),de()}async function Le(){k.disabled=!0,k.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Running demo…',ne(),await g(600),R("Literature Intelligence demo — live PubMed and NEJM searches with AI synthesis"),await B(y,"What are the latest publications on dupilumab?"),R("Searching PubMed and NEJM for dupilumab publications"),M.disabled=!1,await g(500),await I(),await g(4e3),await B(y,"What evidence exists for nirsevimab RSV prevention?"),R("Searching for nirsevimab RSV prevention evidence"),M.disabled=!1,await g(500),await I(),await g(4e3),await B(y,"Tell me about tolebrutinib in multiple sclerosis"),R("Searching for tolebrutinib multiple sclerosis data"),M.disabled=!1,await g(500),await I(),await g(2e3),Ee(),k.disabled=!1,k.innerHTML='<i class="ti ti-player-play"></i> Watch conversation demo'}function je(){const e=document.createElement("div");return e.className="chat-msg chat-msg-agent",e.innerHTML=`
    <div class="chat-agent-avatar"><i class="ti ti-book-2"></i></div>
    <div class="chat-msg-content">
      <div class="chat-msg-bubble chat-agent-bubble">
