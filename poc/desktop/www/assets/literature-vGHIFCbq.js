import"./enhancements-yqlzMlQc.js";/* empty css                     */import{i as ce,s as de,a as ue,b as me,h as pe}from"./narrator-BE1zjzk_.js";import{b as he}from"./orion-bridge-4A-0m1g1.js";import"./taxonomy-B-xcL3O-.js";const N="https://eutils.ncbi.nlm.nih.gov/entrez/eutils",ne='"N Engl J Med"[Journal]',M=document.getElementById("search-input"),P=document.getElementById("search-btn"),B=document.getElementById("results-area"),q=document.getElementById("intel-panel"),A=document.getElementById("signal-feed"),ve=document.getElementById("governance-bar"),R=document.getElementById("conn-pubmed"),X=document.getElementById("conn-nejm"),D=document.getElementById("compare-panel-area"),k=document.getElementById("compare-bar"),U=document.getElementById("memory-section"),Z=document.getElementById("memory-chips"),ee=document.getElementById("memory-clear");let W="all",z=[],te=0,$=[];const H="medverse_lit_memory";document.querySelectorAll(".source-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".source-btn").forEach(i=>i.classList.remove("active")),e.classList.add("active"),W=e.dataset.source,z.length&&oe(z)})});document.querySelectorAll(".quick-tag").forEach(e=>{e.addEventListener("click",()=>{M.value=e.dataset.q,C()})});P.addEventListener("click",C);M.addEventListener("keydown",e=>{e.key==="Enter"&&C()});ge();async function ge(){try{(await fetch(`${N}/einfo.fcgi?db=pubmed&retmode=json`)).ok&&(R.innerHTML='<span class="conn-dot pulse"></span> PubMed E-Utilities — Connected',R.className="conn-chip")}catch{R.innerHTML='<span class="conn-dot"></span> PubMed — Connection failed',R.className="conn-chip error"}X.innerHTML='<span class="conn-dot pulse"></span> NEJM via PubMed — Connected',X.className="conn-chip"}async function C(){const e=M.value.trim();if(e){P.disabled=!0,P.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Searching…',ve.style.display="flex",B.innerHTML=`<div class="search-loading">
  ...
  </div>`);try{const i=await O(e,15);await h(1500);const t=await O(`${e} AND ${ne}`,5),s=new Set(t.map(a=>a.uid)),n=[];t.forEach(a=>{a.isNejm=!0,n.push(a)}),i.forEach(a=>{s.has(a.uid)||(a.isNejm=!1,n.push(a))}),z=n,$=[],G(),oe(n),ye(e,n,i.length,t.length),le(e,n),Me(e,n.length,t.length),Ie(n),B.scrollIntoView({behavior:"smooth",block:"start"})}catch(i){B.innerHTML=`<div class="empty-state">
      <i class="ti ti-alert-triangle"></i>
      <h3>Search error</h3>
      <p>${r(i.message)}</p>
    </div>`}P.disabled=!1,P.innerHTML='<i class="ti ti-search"></i> Search'}}async function O(e,i=15){var c,v;const t=`${N}/esearch.fcgi?db=pubmed&term=${encodeURIComponent(e)}&retmax=${i}&sort=relevance&retmode=json`,a=((c=(await(await fetch(t)).json()).esearchresult)==null?void 0:c.idlist)||[];if(!a.length)return[];const l=`${N}/esummary.fcgi?db=pubmed&id=${a.join(",")}&retmode=json`,d=await(await fetch(l)).json(),u=[];for(const b of a){const p=(v=d.result)==null?void 0:v[b];p&&u.push({uid:p.uid,title:p.title||"Untitled",authors:(p.authors||[]).map(m=>m.name),source:p.source||"",fulljournalname:p.fulljournalname||p.source||"",pubdate:p.pubdate||"",volume:p.volume||"",issue:p.issue||"",pages:p.pages||"",doi:fe(p.elocationid||""),pmid:p.uid})}return u}function fe(e){const i=e.match(/doi:\s*(\S+)/i);return i?i[1]:""}function oe(e){let i=e;if(W==="nejm"?i=e.filter(a=>a.isNejm):W==="pubmed"&&(i=e.filter(a=>!a.isNejm)),!i.length){B.innerHTML=`<div class="empty-state">
      <i class="ti ti-book-off"></i>
      <h3>No results found</h3>
      <p>Try adjusting your search terms or source filter.</p>
    </div>`;return}const t=e.filter(a=>a.isNejm).length,s=e.filter(a=>!a.isNejm).length;B.innerHTML=`
    <div class="results-header">
      <h2>Search Results</h2>
      <span class="results-count">${e.length} articles — ${s} PubMed · ${t} NEJM</span>
    </div>
    <div class="results-list" id="results-list"></div>
  `;const n=document.getElementById("results-list");i.forEach((a,l)=>{const o=document.createElement("div");o.className="article-card";const d=a.authors.length>4?a.authors.slice(0,3).join(", ")+`, et al. (${a.authors.length} authors)`:a.authors.join(", "),u=a.isNejm?"nejm":"pubmed",c=a.isNejm?"NEJM":"PubMed",v=`https://pubmed.ncbi.nlm.nih.gov/${a.pmid}/`,b=a.doi?`https://doi.org/${a.doi}`:"";o.innerHTML=`
      <div class="compare-check" data-pmid="${a.pmid}" title="Select for comparison"><i class="ti ti-check"></i></div>
      <span class="article-source-tag ${u}"><span class="source-dot ${u}"></span> ${c}</span>
      <div class="article-title"><a href="${r(v)}" target="_blank" rel="noopener">${a.title}</a></div>
      <div class="article-authors">${r(d)}</div>
      <div class="article-meta">
        <span class="article-meta-item article-journal"><i class="ti ti-notebook"></i> ${r(a.fulljournalname)}</span>
        <span class="article-meta-item"><i class="ti ti-calendar"></i> ${r(a.pubdate)}</span>
        ${a.volume?`<span class="article-meta-item">${r(a.volume)}${a.issue?`(${r(a.issue)})`:""}${a.pages?`:${r(a.pages)}`:""}</span>`:""}
      </div>
      <div class="article-pmid">PMID: ${r(a.pmid)}</div>
      <div class="article-actions">
        ...
        <button class="article-action fetch-abstract-btn" data-pmid="${a.pmid}" data-idx="${l}"><i class="ti ti-file-text"></i> Fetch Abstract</button>
      </div>
      <div class="article-abstract-area" id="abstract-${a.pmid}"></div>
    `,n.appendChild(o)}),n.querySelectorAll(".fetch-abstract-btn").forEach(a=>{a.addEventListener("click",()=>be(a.dataset.pmid))}),n.querySelectorAll(".compare-check").forEach(a=>{a.addEventListener("click",()=>Ee(a.dataset.pmid,a))})}async function be(e){const i=document.getElementById(`abstract-${e}`);if(i){i.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:8px 0"><i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Fetching abstract from PubMed…</p>';try{const t=`${N}/efetch.fcgi?db=pubmed&id=${e}&rettype=abstract&retmode=text`,n=await(await fetch(t)).text(),a=n.match(/\n\n([\s\S]*?)(?:\n\n(?:Copyright|DOI|PMID|©)|\n\nPMID:)/i),l=a?a[1].trim():n.trim();l&&l.length>50?i.innerHTML=`<div class="article-abstract" style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border)">${r(l)}</div>
        <button class="article-expand" onclick="this.previousElementSibling.classList.toggle('expanded');this.textContent=this.textContent==='Show more'?'Show less':'Show more'">Show more</button>`:i.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:8px 0">Abstract not available for this article.</p>'}catch{i.innerHTML='<p style="font-size:12px;color:var(--danger);padding:8px 0">Failed to fetch abstract.</p>'}}}function ye(e,i,t,s){const n={},a={},l={};i.forEach(m=>{const g=m.source||m.fulljournalname||"Unknown";n[g]=(n[g]||0)+1;const j=m.pubdate.match(/(\d{4})/);if(j){const f=j[1];a[f]=(a[f]||0)+1}m.authors.forEach(f=>{l[f]=(l[f]||0)+1})});const o=Object.entries(n).sort((m,g)=>g[1]-m[1]).slice(0,5),d=o.length?o[0][1]:1,u=Object.entries(l).sort((m,g)=>g[1]-m[1]).slice(0,8),c=Object.entries(a).sort((m,g)=>m[0]-g[0]),v=c.length?Math.max(...c.map(m=>m[1])):1,b=["#7a00e6","#aa46a3","#f9c851","#60a5fa","#34d399"];let p="";p+=`<div class="intel-card">
    <div class="intel-card-header"><i class="ti ti-chart-bar"></i> Search Analytics</div>
    <div class="intel-stat-row"><span class="intel-stat-label">Total articles</span><span class="intel-stat-value">${i.length}</span></div>
    ...
    <div class="intel-stat-row"><span class="intel-stat-label">Date range</span><span class="intel-stat-value">${c.length?c[0][0]+"–"+c[c.length-1][0]:"—"}</span></div>
      ...
      <div class="trend-chart-wrapper">
        <div class="trend-chart">
          ${c.map(([m,g])=>`
            <div class="trend-bar" style="height:${Math.max(g/v*100,8)}%" title="${m}: ${g} articles">
      ...
      ${o.map(([m,g],j)=>`
        <div class="journal-row">
          ...
          <div class="journal-bar"><div class="journal-bar-fill" style="width:${g/d*100}%;background:${b[j%b.length]}"></div></div>
          <span class="journal-count">${g}</span>
        </div>
      `).join("")}
    </div>`),u.length&&(p+=`<div class="intel-card">
      <div class="intel-card-header"><i class="ti ti-users"></i> Key Authors</div>
      <div style="display:flex;flex-wrap:wrap;gap:4px">
        ${u.map(([m,g])=>`<span class="author-chip">${r(m)} (${g})</span>`).join("")}
      </div>
    ...
    <p style="font-size:12px;color:var(--text-secondary);line-height:1.5;margin-bottom:6px">
      <strong>${i.length} publications</strong> retrieved for "<em>${r(e)}</em>".
    ...
    ${u.length>=2?`<p style="font-size:12px;color:var(--text-secondary);line-height:1.5">
      ...
      <div class="signal-row"><span class="signal-label">Sources</span><span class="signal-value">${i.length} PubMed · ${t} NEJM</span></div>
    ...
    `,A.insertBefore(v,A.firstChild)}he({topic:`Literature Search — ${e.length>40?e.slice(0,38)+"…":e}`,intent:d,diseaseArea:l,depth:o,orionAction:u,queries:[e],contentAccessed:i.slice(0,3).map(c=>c.title||"PubMed Article"),sessionDuration:Math.floor(Math.random()*8)+3,_source:"Literature Intelligence"})}function $e(e){const i=e.toLowerCase(),t=[[/atopic|eczema|dermatitis|dupilumab|dupixent/,"Atopic Dermatitis"],[/asthma|airway|tezepelumab/,"Asthma"],[/crohn|ibd|ulcerative colitis|duvakitug/,"Inflammatory Bowel Disease"],[/rsv|nirsevimab|beyfortus/,"RSV Prevention"],[/hemophilia|fitusiran/,"Hemophilia"],[/multiple sclerosis|tolebrutinib/,"Multiple Sclerosis"],[/insulin|glargine|lantus|toujeo|diabetes/,"Diabetes"],[/psoriasis/,"Psoriasis"],[/oncology|cancer|tumor/,"Oncology"],[/vaccine|immunization/,"Vaccines"],[/rare disease|fabry|gaucher|pompe/,"Rare Diseases"]];for(const[s,n]of t)if(s.test(i))return n;return"Cross-TA / General"}function r(e){const i=document.createElement("div");return i.textContent=e,i.innerHTML}function h(e){return new Promise(i=>setTimeout(i,e))}document.getElementById("chat-panel");const x=document.getElementById("chat-messages"),w=document.getElementById("chat-input"),T=document.getElementById("chat-send"),S=document.getElementById("chat-suggestions");w&&T&&(T.addEventListener("click",()=>J()),w.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),J())}),w.addEventListener("input",()=>{T.disabled=!w.value.trim()}));S&&S.querySelectorAll(".chat-suggestion").forEach(e=>{e.addEventListener("click",()=>{w.value=e.dataset.q,J()})});const ie={patterns:[{match:/what.*(latest|recent|new).*(dupilumab|dupixent)/i,query:"dupilumab 2024 2025 2026",synthesize:e=>`Based on my search of PubMed and NEJM, here are the most recent dupilumab publications:
...
**Evidence gap:** No published head-to-head data for upadacitinib vs dupilumab. This is a frequently requested comparison from HCPs (67 interaction signals this quarter).`},{match:/safety.*(pediatric|children|child|kids)/i,query:"dupilumab pediatric safety long-term children",synthesize:e=>`Pediatric dupilumab safety evidence from PubMed/NEJM:
...
**Evidence status:** Phase 3 data emerging. Watch for HERCULES (non-relapsing SPMS) and GEMINI (RMS) trial readouts.`}],fallback:(e,i)=>{if(!i.length)return`I searched PubMed and NEJM for "${e}" but found no matching articles. Try refining your query with more specific medical terms, drug names, or disease areas.`;const t=i.slice(0,3).map((l,o)=>`${o+1}. **${l.title}** — *${l.fulljournalname}* (${l.pubdate}) [PMID: ${l.pmid}]`).join(`

`),s=i.filter(l=>l.isNejm).length;let n="";const a=localStorage.getItem(H);if(a){const o=JSON.parse(a).filter(d=>d.query.toLowerCase()!==e.toLowerCase()).slice(0,2);o.length&&(n=`

**Agent memory:** You previously searched for ${o.map(d=>`"${d.query}" (${d.count} results)`).join(" and ")}. I can cross-reference these topics if you'd like a combined analysis.`)}return`I found ${i.length} articles across PubMed${s?` and NEJM (${s} high-impact)`:""} for your query:

${t}

${i.length>3?`Plus ${i.length-3} more articles. `:""}Use the search bar above to see all results with full metadata, or ask me a more specific follow-up question.${n}`}};async function J(){const e=w.value.trim();if(!e)return;w.value="",T.disabled=!0,S&&(S.style.display="none"),_("user",e);const i=Se();let t=null;for(const n of ie.patterns)if(n.match.test(e)){t=n;break}const s=t?t.query:we(e);try{const n=await O(s,8);await h(1200);const a=await O(`${s} AND ${ne}`,3),l=new Set(a.map(u=>u.uid));a.forEach(u=>{u.isNejm=!0}),n.forEach(u=>{l.has(u.uid)&&(u.isNejm=!0)});const o=[...a,...n.filter(u=>!l.has(u.uid))];i.remove();const d=t?t.synthesize(o):ie.fallback(e,o);_("agent",d,o),le(s,o)}catch(n){i.remove(),_("agent",`I encountered an error searching PubMed: ${n.message}. Please try again in a moment — NCBI rate-limits requests to 3 per second.`)}}function we(e){return e.replace(/\b(what|how|does|is|are|the|can|you|find|tell|me|about|give|show|latest|recent|any|published|evidence|data|for|on|in|of|with|and|or|please)\b/gi,"").replace(/[?!.,]/g,"").replace(/\s+/g," ").trim()}function _(e,i,t){const s=document.createElement("div");if(s.className=`chat-msg chat-msg-${e}`,e==="user")s.innerHTML=`<div class="chat-msg-bubble chat-user-bubble">${r(i)}</div>`;else{let n=r(i);n=n.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),n=n.replace(/\*(.+?)\*/g,"<em>$1</em>"),n=n.replace(/\[PMID: (\d+)\]/g,'<a class="chat-pmid-link" href="https://pubmed.ncbi.nlm.nih.gov/$1/" target="_blank" rel="noopener">PMID: $1</a>'),n=n.replace(/^(\d+)\./gm,"<br>$1."),n=n.replace(/^- (.+)/gm,"<br>• $1");const a=t&&t.length>0?`<div class="chat-citations">
      ...
      <div class="chat-msg-content">
        ...
        ${a}
      </div>`}x.appendChild(s),x.scrollTop=x.scrollHeight}function Y(){const e=localStorage.getItem(H),i=e?JSON.parse(e):[];if(!i.length){U&&(U.style.display="none");return}U.style.display="block",Z.innerHTML="",i.slice(0,8).forEach(t=>{const s=document.createElement("span");s.className="memory-chip",s.title=`${t.count} results · ${t.nejm} NEJM · ${t.date}`,s.innerHTML=`<i class="ti ti-history"></i> ${r(t.query)} <span class="memory-chip-time">${t.ago}</span>`,s.addEventListener("click",()=>{M.value=t.query,C()}),Z.appendChild(s)})}function Me(e,i,t){const s=localStorage.getItem(H);let n=s?JSON.parse(s):[];n=n.filter(l=>l.query.toLowerCase()!==e.toLowerCase());const a=new Date;n.unshift({query:e,count:i,nejm:t,date:a.toLocaleDateString("en-US",{month:"short",day:"numeric"}),ago:"just now",ts:a.getTime()}),n=n.slice(0,20),n.forEach(l=>{const o=Math.floor((a.getTime()-l.ts)/6e4);o<1?l.ago="just now":o<60?l.ago=`${o}m ago`:o<1440?l.ago=`${Math.floor(o/60)}h ago`:l.ago=`${Math.floor(o/1440)}d ago`}),localStorage.setItem(H,JSON.stringify(n)),Y()}Y();ee&&ee.addEventListener("click",()=>{localStorage.removeItem(H),Y()});function Ee(e,i){var s;const t=$.indexOf(e);if(t>-1)$.splice(t,1),i.classList.remove("selected");else{if($.length>=2){const n=$.shift();(s=document.querySelector(`.compare-check[data-pmid="${n}"]`))==null||s.classList.remove("selected")}$.push(e),i.classList.add("selected")}G()}function G(){if(k){if($.length<2){k.style.display=$.length===1?"flex":"none",$.length===1&&(k.innerHTML=`<div class="compare-bar">
  ...
  </div>`,k.style.display="block"}}window.clearCompare=function(){$=[],document.querySelectorAll(".compare-check.selected").forEach(e=>e.classList.remove("selected")),G()};window.runComparison=async function(){var g,j;const e=$.map(f=>z.find(I=>I.uid===f||I.pmid===f)).filter(Boolean);if(e.length<2)return;D.innerHTML=`<div class="compare-panel">
  ...
  </div>`,D.scrollIntoView({behavior:"smooth",block:"start"});const i=[];for(const f of e)try{const I=`${N}/efetch.fcgi?db=pubmed&id=${f.pmid}&rettype=abstract&retmode=text`,Q=(await(await fetch(I)).text()).match(/\n\n([\s\S]*?)(?:\n\n(?:Copyright|DOI|PMID|©)|\n\nPMID:)/i);i.push(Q?Q[1].trim():"Abstract not available."),i.length<e.length&&await h(1e3)}catch{i.push("Failed to fetch abstract.")}const t=e[0],s=e[1],n=i[0],a=i[1],l=t.fulljournalname===s.fulljournalname,o=((g=t.pubdate.match(/\d{4}/))==null?void 0:g[0])||"?",d=((j=s.pubdate.match(/\d{4}/))==null?void 0:j[0])||"?",u=Math.abs(parseInt(o)-parseInt(d));let c="";if(l)c=`Both articles published in <strong>${r(t.fulljournalname)}</strong>`,u>0&&(c+=` — ${u}-year gap suggests evolving evidence base`),c+=". ";else{const f=t.isNejm,I=s.isNejm;f&&!I?c=`Article 1 is from <strong>NEJM</strong> (highest impact) vs ${r(s.fulljournalname)}. `:!f&&I?c=`Article 2 is from <strong>NEJM</strong> (highest impact) vs ${r(t.fulljournalname)}. `:c=`Cross-journal comparison: <strong>${r(t.source)}</strong> vs <strong>${r(s.source)}</strong>. `}const v=t.authors.length,b=s.authors.length;(v>10||b>10)&&(c+=`Large author list (${Math.max(v,b)} authors) suggests multi-center trial data. `),c+="Review both abstracts to assess study design, population, and endpoints for comparative evidence strength.";const p=f=>f.isNejm?"nejm":"pubmed",m=f=>f.isNejm?"NEJM":"PubMed";D.innerHTML=`<div class="compare-panel">
    <div class="compare-panel-header">
    ...
    <div class="compare-grid">
      ...
      <div class="compare-col">
        ...
        <div class="compare-field"><div class="compare-field-label">Abstract</div><div class="compare-abstract">${r(a)}</div></div>
      </div>
    </div>
    <div class="compare-ai-row">
      ...
      <div class="compare-ai-text">${c}</div>
          ...
          </div>`}else s.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:8px 0"><span class="deep-dive-badge"><i class="ti ti-sparkles"></i> Deep-Dive</span> Abstract not available from PubMed for this article.</p>'}catch{s.innerHTML=""}}}}const E=document.getElementById("demo-search-btn");E&&E.addEventListener("click",V);async function F(e,i){e.value="";for(let t=0;t<i.length;t++)e.value+=i[t],await h(20+Math.random()*25)}function re(){x.innerHTML=`<div class="chat-msg chat-msg-agent">
    <div class="chat-agent-avatar"><i class="ti ti-book-2"></i></div>
    <div class="chat-msg-content">
    ...
    <button class="chat-suggestion" data-q="Tell me about tolebrutinib in multiple sclerosis">Tolebrutinib in MS</button>`,S.style.display="flex",w.value="",T.disabled=!0,S.querySelectorAll(".chat-suggestion").forEach(e=>{e.addEventListener("click",()=>{w.value=e.dataset.q,J()})})}const ae=document.getElementById("chat-clear-btn");ae&&ae.addEventListener("click",re);const L=document.getElementById("demo-chat-btn");L&&L.addEventListener("click",V);async function y(e){const i=document.getElementById("demo-narrator");i&&(i.innerHTML=`<i class="ti ti-sparkles"></i> ${e}`,ce()&&i.classList.add("visible"),de(),await ue(e))}function xe(){const e=document.getElementById("demo-narrator");e&&e.classList.remove("visible"),me(),pe()}let K=!1;async function V(){if(K)return;K=!0;const e=document.getElementById("run-demo");e&&(e.disabled=!0,e.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Running...'),E&&(E.disabled=!0,E.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Running…'),L&&(L.disabled=!0,L.innerHTML='<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Running…');const i=document.querySelector(".main-panel");i&&i.scrollTo({top:0,behavior:"smooth"}),await h(500),await y("Literature Intelligence — live PubMed and NEJM search with AI synthesis and interaction signal integration");const t=document.getElementById("governance-bar");t&&(t.style.display="flex",t.scrollIntoView({behavior:"smooth",block:"center"}),await h(600)),await y("Governance layer active — every search is logged, sourced, and compliance-stamped"),await y("Let's search PubMed — querying dupilumab atopic dermatitis long-term safety"),M.scrollIntoView({behavior:"smooth",block:"center"}),await h(400),await F(M,"dupilumab atopic dermatitis long-term safety"),await h(600),await C(),await h(4e3),await y("Results include NEJM articles with auto deep-dive summaries and Sanofi pipeline intelligence");const s=document.querySelector(".deep-dive-badge");s&&(s.scrollIntoView({behavior:"smooth",block:"center"}),await h(600),await y("The agent already pulled the abstract and wrote this summary — no click required"),await h(1200));const n=document.getElementById("intel-panel");n&&(n.scrollIntoView({behavior:"smooth",block:"center"}),await h(800),await y("Intelligence panel — publication trends, top journals, and key authors extracted automatically"),await h(1500)),await y("Filtering to NEJM only — the highest impact journal results");const a=document.querySelector('.source-btn[data-source="nejm"]');a&&(a.click(),await h(800)),await y("NEJM filter applied — showing only New England Journal of Medicine publications");const l=document.querySelector('.source-btn[data-source="all"]');l&&l.click(),await h(400);const o=document.getElementById("memory-section");o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),await h(600),await y("Agent memory — the system remembers your search history across sessions"),await h(1e3)),M.scrollIntoView({behavior:"smooth",block:"center"}),await h(400),await y("Second search — nirsevimab RSV prevention to demonstrate cross-TA capabilities"),M.value="",await F(M,"nirsevimab RSV prevention infants"),await h(600),await C(),await h(4e3),await y("Each search generates an intelligence signal for medical insights and analytics"),await y("The same agent answers questions directly in chat, with live PubMed searches behind every reply"),re(),await h(400),await F(w,"Tell me about tolebrutinib in multiple sclerosis"),await h(600),T.disabled=!1,await J(),await h(5e3),await y("Live PubMed results with citations and source links — the agent covers every Sanofi therapeutic area this way"),i&&i.scrollTo({top:0,behavior:"smooth"}),await h(500),await y("Literature Intelligence — live PubMed, NEJM, AI synthesis, and interaction signals in one platform"),xe(),K=!1,e&&(e.disabled=!1,e.innerHTML='<i class="ti ti-player-play"></i> Play Demo'),E&&(E.disabled=!1,E.innerHTML='<i class="ti ti-player-play"></i> Watch literature search demo'),L&&(L.disabled=!1,L.innerHTML='<i class="ti ti-player-play" style="font-size:13px"></i> Demo')}const se=document.getElementById("run-demo");se&&se.addEventListener("click",V);window.location.hash==="#autoplay"&&(window.location.hash="",setTimeout(V,600));function Se(){const e=document.createElement("div");return e.className="chat-msg chat-msg-agent",e.innerHTML=`
    <div class="chat-agent-avatar"><i class="ti ti-book-2"></i></div>
    <div class="chat-msg-content">
      <div class="chat-msg-bubble chat-agent-bubble">
