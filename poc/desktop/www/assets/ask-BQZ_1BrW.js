...
`;function ae(e){var i;return(i=navigator.clipboard)!=null&&i.writeText?navigator.clipboard.writeText(e):new Promise((n,c)=>{const t=document.createElement("textarea");t.value=e,t.setAttribute("readonly",""),t.style.position="fixed",t.style.opacity="0",document.body.appendChild(t),t.select();const s=document.execCommand("copy");t.remove(),s?n():c(new Error("copy failed"))})}function x(e,i){const n=i.trim().toLowerCase(),c=ee.map(t=>({...t,questions:t.questions.filter(s=>!n||s.q.toLowerCase().includes(n)||t.label.toLowerCase().includes(n))})).filter(t=>t.questions.length);if(!c.length){e.innerHTML=`<div class="mv-prompts-none">No suggested question matches “${g(i)}”.</div>`;return}e.innerHTML=c.map(t=>`
    <div class="mv-prompts-cat">
      <div class="mv-prompts-cat-label"><i class="ti ti-${g(t.icon)}"></i> ${g(t.label)}</div>
      ${t.blurb?`<div class="mv-prompts-cat-blurb">${g(t.blurb)}</div>`:""}
      ${t.questions.map(s=>`
    ...
    </div>`).join("")}function ne(){const e=document.getElementById("ask-form"),i=document.getElementById("ask-input");if(!e||!i)return;const n=document.createElement("style");n.textContent=ie,document.head.appendChild(n);const c=document.createElement("div");c.className="mv-prompts-trigger-row",c.innerHTML=`
    ...
    </div>`,document.body.appendChild(t);const s=t.querySelector(".mv-prompts-body"),l=t.querySelector(".mv-prompts-filter");x(s,"");function r(){t.classList.add("open"),l.value="",x(s,""),requestAnimationFrame(()=>l.focus())}function d(){t.classList.remove("open")}c.querySelector("#mv-prompts-trigger").addEventListener("click",r),t.querySelector(".mv-prompts-close").addEventListener("click",d),t.addEventListener("click",p=>{p.target===t&&d()}),document.addEventListener("keydown",p=>{p.key==="Escape"&&t.classList.contains("open")&&d()}),l.addEventListener("input",()=>x(s,l.value)),s.addEventListener("click",p=>{const a=p.target.closest(".mv-prompts-run");if(a){i.value=a.getAttribute("data-q"),d(),e.requestSubmit?e.requestSubmit():e.submit();return}const u=p.target.closest(".mv-prompts-pick");if(u){i.value=u.getAttribute("data-q"),d(),i.focus();return}const v=p.target.closest(".mv-prompts-copy");v&&ae(v.getAttribute("data-q")).then(()=>{v.classList.add("copied"),v.innerHTML='<i class="ti ti-check"></i>',setTimeout(()=>{v.classList.remove("copied"),v.innerHTML='<i class="ti ti-copy"></i>'},1600),window.mvToast&&window.mvToast("Question copied to clipboard","success",2e3)}).catch(()=>{window.mvToast&&window.mvToast("Could not access the clipboard — select and copy manually","warning")})})}const se={podcast:"microphone-2",video:"player-play",infographic:"chart-infographic",article:"file-text"},o=e=>String(e??"").replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i]);function oe(e){let i=o(e);return i=i.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),i=i.replace(/\*(.+?)\*/g,"<em>$1</em>"),i=i.replace(/^### (.+)$/gm,"<h3>$1</h3>"),i=i.replace(/^- (.+)$/gm,"<li>$1</li>"),i=i.replace(/(<li>.*<\/li>)/s,"<ul>$1</ul>"),i=i.replace(/\[(\d+)\]/g,'<span class="cite-ref">$1</span>'),i=i.replace(/((?:^\|.*\|\s*$\n?)+)/gm,n=>{const c=n.trim().split(`
`).filter(r=>r.trim().startsWith("|"));if(c.length<2)return n;const t=r=>r.split("|").slice(1,-1).map(d=>d.trim()),s=t(c[0]),l=c.slice(2).map(t);return l.length?`<table><thead><tr>${s.map(r=>`<th>${r}</th>`).join("")}</tr></thead><tbody>${l.map(r=>`<tr>${r.map(d=>`<td>${d}</td>`).join("")}</tr>`).join("")}</tbody></table>`:n}),i.split(/\n{2,}/).map(n=>/^<(h3|ul|table)/.test(n.trim())?n:`<p>${n.replace(/\n/g,"<br>")}</p>`).join("")}const re=(document.body.dataset.modules||"").split(",").map(e=>e.trim()).filter(Boolean),ce=new URLSearchParams(location.search);let h=(ce.get("q")||"").trim();const m=document.getElementById("ask-input"),$=document.getElementById("ask-form"),S=document.getElementById("ask-results"),L=document.getElementById("ask-query-echo");m&&(m.value=h);$&&$.addEventListener("submit",e=>{e.preventDefault();const i=m.value.trim();i&&(location.search="?q="+encodeURIComponent(i))});const le=["What are my options for a patient with moderate-to-severe atopic dermatitis who failed topicals?","Early detection and screening for type 1 diabetes","What trials are recruiting for atopic dermatitis?","Who is my MSL for dermatology?","Cardiac manifestations and biomarkers in Fabry disease","How do I request compassionate use for an unapproved medicine?"],b=document.querySelector(".ask-wrap"),w=document.querySelector(".ask-echo");function N(){var p;if(!h){b&&b.classList.add("ask-landing"),w&&(w.style.display="none"),document.title="Ask MedVerse",S.innerHTML=`
      <div class="ask-examples">
        <div class="ask-examples-label">Or try one of these</div>
        ${le.map(a=>`<button type="button" class="ask-example" data-q="${o(a)}">${o(a)}</button>`).join("")}
      </div>`,S.querySelectorAll(".ask-example").forEach(a=>{a.addEventListener("click",()=>{const u=a.getAttribute("data-q");m&&(m.value=u),location.search="?q="+encodeURIComponent(u)})}),m&&m.focus();return}b&&b.classList.remove("ask-landing"),w&&(w.style.display=""),L&&(L.textContent=h),document.title=`${h} — Ask MedVerse`;const e=Z(h,re),i=e.agents.map(a=>`
    <a class="ask-agent-card" href="${o(q(a,h))}">
    ...
    <div class="ask-section">
      ...
      ${e.trials.map(a=>`
        <a class="ask-trial" href="${o(F(a))}" target="_blank" rel="noopener">
    ...
    <div class="ask-section">
      ...
      ${e.resources.map(a=>`
        <a class="ask-resource" href="${o(a.url)}" target="_blank" rel="noopener">
          <div class="ask-resource-icon"><i class="ti ti-${se[a.contentType]||"file-text"}"></i></div>
          <div>
    ...
    <div class="ask-answer-card">
      ...
      <div class="ask-answer-body rendered">${oe(e.answer.answer)}</div>
      ${n}
    ...
    <div class="ask-answer-card">
      ...
      <div class="ask-answer-body">
        <p>${o(a.blurb)}.</p>
        <p style="margin-top:10px;"><a class="ask-action-btn" href="${o(q(a,h))}"><i class="ti ti-arrow-right"></i> Open ${o(a.name)}</a></p>
      </div>
    </div>`}else l=`
    <div class="ask-answer-card">
      <div class="ask-answer-head">
    ...
    </div>`;let r=null;if(e.answerMode==="evidence"&&e.answer.signal)r=e.answer.signal;else if(e.agents.length){const a=e.agents[0];r={topic:h,intent:`${a.name} request`,diseaseArea:((p=e.answer.signal)==null?void 0:p.diseaseArea)||"General",orionAction:`Routed to ${a.name}. Logged as an HCP-initiated ${a.name.toLowerCase()} request.`}}const d=r?`
