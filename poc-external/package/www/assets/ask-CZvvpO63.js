...
`;function $(e){return new Promise((t,n)=>{const r=document.createElement("textarea");r.value=e,r.setAttribute("readonly",""),r.style.position="fixed",r.style.opacity="0",document.body.appendChild(r),r.select();let i=!1;try{i=document.execCommand("copy")}catch{}r.remove(),i?t():n(new Error("copy failed"))})}function ne(e){var t;return(t=navigator.clipboard)!=null&&t.writeText?navigator.clipboard.writeText(e).catch(()=>$(e)):$(e)}function x(e,t){const n=t.trim().toLowerCase(),r=te.map(i=>({...i,questions:i.questions.filter(s=>!n||s.q.toLowerCase().includes(n)||i.label.toLowerCase().includes(n))})).filter(i=>i.questions.length);if(!r.length){e.innerHTML=`<div class="mv-prompts-none">No suggested question matches “${g(t)}”.</div>`;return}e.innerHTML=r.map(i=>`
    ...
    </div>`).join("")}function se(){const e=document.getElementById("ask-form"),t=document.getElementById("ask-input");if(!e||!t)return;const n=document.createElement("style");n.textContent=ae,document.head.appendChild(n);const r=document.createElement("div");r.className="mv-prompts-trigger-row",r.innerHTML=`
    ...
    </div>`,document.body.appendChild(i);const s=i.querySelector(".mv-prompts-body"),l=i.querySelector(".mv-prompts-filter");x(s,"");function c(){i.classList.add("open"),l.value="",x(s,""),requestAnimationFrame(()=>l.focus())}function d(){i.classList.remove("open")}r.querySelector("#mv-prompts-trigger").addEventListener("click",c),i.querySelector(".mv-prompts-close").addEventListener("click",d),i.addEventListener("click",p=>{p.target===i&&d()}),document.addEventListener("keydown",p=>{p.key==="Escape"&&i.classList.contains("open")&&d()}),l.addEventListener("input",()=>x(s,l.value)),s.addEventListener("click",p=>{const a=p.target.closest(".mv-prompts-run");if(a){t.value=a.getAttribute("data-q"),d(),e.requestSubmit?e.requestSubmit():e.submit();return}const u=p.target.closest(".mv-prompts-pick");if(u){t.value=u.getAttribute("data-q"),d(),t.focus();return}const v=p.target.closest(".mv-prompts-copy");v&&ne(v.getAttribute("data-q")).then(()=>{v.classList.add("copied"),v.innerHTML='<i class="ti ti-check"></i>',setTimeout(()=>{v.classList.remove("copied"),v.innerHTML='<i class="ti ti-copy"></i>'},1600),window.mvToast&&window.mvToast("Question copied to clipboard","success",2e3)}).catch(()=>{window.mvToast&&window.mvToast("Could not access the clipboard — select and copy manually","warning")})})}const oe={podcast:"microphone-2",video:"player-play",infographic:"chart-infographic",article:"file-text"},o=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]);function re(e){let t=o(e);return t=t.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),t=t.replace(/\*(.+?)\*/g,"<em>$1</em>"),t=t.replace(/^### (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/^- (.+)$/gm,"<li>$1</li>"),t=t.replace(/(<li>.*<\/li>)/s,"<ul>$1</ul>"),t=t.replace(/\[(\d+)\]/g,'<span class="cite-ref">$1</span>'),t=t.replace(/((?:^\|.*\|\s*$\n?)+)/gm,n=>{const r=n.trim().split(`
`).filter(c=>c.trim().startsWith("|"));if(r.length<2)return n;const i=c=>c.split("|").slice(1,-1).map(d=>d.trim()),s=i(r[0]),l=r.slice(2).map(i);return l.length?`<table><thead><tr>${s.map(c=>`<th>${c}</th>`).join("")}</tr></thead><tbody>${l.map(c=>`<tr>${c.map(d=>`<td>${d}</td>`).join("")}</tr>`).join("")}</tbody></table>`:n}),t.split(/\n{2,}/).map(n=>/^<(h3|ul|table)/.test(n.trim())?n:`<p>${n.replace(/\n/g,"<br>")}</p>`).join("")}const ce=(document.body.dataset.modules||"").split(",").map(e=>e.trim()).filter(Boolean),le=new URLSearchParams(location.search);let h=(le.get("q")||"").trim();const m=document.getElementById("ask-input"),L=document.getElementById("ask-form"),S=document.getElementById("ask-results"),R=document.getElementById("ask-query-echo");m&&(m.value=h);L&&L.addEventListener("submit",e=>{e.preventDefault();const t=m.value.trim();t&&(location.search="?q="+encodeURIComponent(t))});const de=["What are my options for a patient with moderate-to-severe atopic dermatitis who failed topicals?","Early detection and screening for type 1 diabetes","What trials are recruiting for atopic dermatitis?","Who is my MSL for dermatology?","Cardiac manifestations and biomarkers in Fabry disease","How do I request compassionate use for an unapproved medicine?"],b=document.querySelector(".ask-wrap"),w=document.querySelector(".ask-echo");function P(){var p;if(!h){b&&b.classList.add("ask-landing"),w&&(w.style.display="none"),document.title="Ask MedVerse",S.innerHTML=`
      <div class="ask-examples">
        <div class="ask-examples-label">Or try one of these</div>
        ${de.map(a=>`<button type="button" class="ask-example" data-q="${o(a)}">${o(a)}</button>`).join("")}
      </div>`,S.querySelectorAll(".ask-example").forEach(a=>{a.addEventListener("click",()=>{const u=a.getAttribute("data-q");m&&(m.value=u),location.search="?q="+encodeURIComponent(u)})}),m&&m.focus();return}b&&b.classList.remove("ask-landing"),w&&(w.style.display=""),R&&(R.textContent=h),document.title=`${h} — Ask MedVerse`;const e=ee(h,ce),t=e.agents.map(a=>`
    ...
    <div class="ask-section">
      ...
      ${e.resources.map(a=>`
        <a class="ask-resource" href="${o(a.url)}" target="_blank" rel="noopener">
          <div class="ask-resource-icon"><i class="ti ti-${oe[a.contentType]||"file-text"}"></i></div>
          <div>
    ...
    <div class="ask-answer-card">
      ...
      <div class="ask-answer-body rendered">${re(e.answer.answer)}</div>
      ${n}
